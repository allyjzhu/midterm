...
let levelData = [levelA, levelB, levelC];
...
function updateTime(){ 
  const timeElapsed =  millis() - timeStarted;
...
function drawTime(timeRemaining){
  const time = millisToMinutesAndSeconds(timeRemaining)
  textSize(20);
  // text("score: ", 10,30);
  if(timeRemaining < 5100){
    fill(255, 0, 0);
  } else {
    fill(0,0,0);
  }
  text("time left: ", width -150,0);
  text(time, width - 70, 0);
}
